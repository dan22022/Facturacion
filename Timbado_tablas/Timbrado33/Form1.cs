...
namespace Timbrado33
{
    ...
    public partial class Form1 : Form
    {
        ...
        public void Principal()//Metodo principal invocador de los demas metodos.
        {
            BD.Open();
            DataTable datos = BD.Datos_Tabla_Banderas();// llamada al metodo Datis_Tabla_Banderas

            for (int i = 0; i < datos.Rows.Count; i++)
            {
                DataRow row = datos.Rows[i];
                //Declaracion de variables de estados de bandera e ID
                ...
                if (estado_correo == 0)//Comprobacion del estado de envio de la factura
                {
                    Factura(id);//Llamada al metodo de factura.
                }
            }                
        }
        private  void Factura(int id)//Metodo encargado de la generacion del Json que porsteriormente sera timbrado y devuelto
        {
            ...
            cfdi.SubTotal = datos[6]; // máximo 6 decimales
